...
Game.spawnBoss = function() {
    ...
    let type = 'normal';
    let hp = 100 + (this.currentLevelIndex * 10);
    let w = 140; let h = 100; 
    ...
    this.boss = {
        ...
        type: type, phase: 'enter', shootTimer: 0, minionTimer: 0, imgSrc: imgSrc
    };
...
Game.spawnBossMinion = function() {
    ...
    for(let i = -1; i <= 1; i += 2) {
        this.enemies.push({
            x: this.boss.x + (i * 60), y: this.boss.y + 50,
...
Game.updateBoss = function(moveMult) {
    ...
    if(boss.phase === 'enter') {
        ...
        for(let b of this.bullets) {
            if(b.y > boss.y && b.y < boss.y + 250 && Math.abs(b.x - boss.x) < boss.w + 20) {
                 dodgeVel = (boss.x < b.x) ? -4 : 4; break; 
            }
        }
        
        // 2. AI: Movement
        boss.vx += dodgeVel * 0.5; 
        if(boss.x < 100) boss.vx += 0.2;
        else if(boss.x > canvas.width - 100) boss.vx -= 0.2;
        else { boss.vx += (Math.random() - 0.5) * 0.5; boss.vx += Math.sin(this.frameCount * 0.05) * 0.1; }
        
        boss.vx *= 0.95; 
        if(boss.vx > 6) boss.vx = 6; if(boss.vx < -6) boss.vx = -6;
        ...
        if(boss.shootTimer > fireRate) {
            ...
            for(let k=0; k<3; k++) {
                setTimeout(() => {
                    if(!Game.boss) return; // Safety check
                    const angle = Math.atan2(Game.player.y - Game.boss.y, Game.player.x - Game.boss.x);
                    const spread = (Math.random() - 0.5) * 0.3;
                    Game.enemies.push({
                        x: Game.boss.x, y: Game.boss.y + 50, w: 20, h: 20, letter: '', isTrap: true, hp: 1, 
                        isBossBullet: true, vy: Math.sin(angle + spread) * 7, vx: Math.cos(angle + spread) * 7,
        ...
        if (this.frameCount % 120 === 0) { 
            ...
            if(Math.random() < 0.4) this.spawnHeart(dropX, -50);
            else this.spawnPowerup(dropX, -50);
        }
    }
...
Game.checkBossCollisions = function() {
    ...
    if(this.player.invulnerable === 0 && Math.abs(this.player.x - this.boss.x) < (this.boss.w/2 + this.player.w/2) && Math.abs(this.player.y - this.boss.y) < (this.boss.h/2 + this.player.h/2)) {
         if(this.powerupState.shield > 0) { 
             this.powerupState.shield = 0; // Set to 0
