...
Game.spawnBoss = function() {
    ...
    const isMid = this.currentLevelIndex === 4;   
    AudioSys.playBossLaugh(isFinal);

    if (typeof this.spawnPartner === 'function') {
        this.spawnPartner();
    }
    ...
    if (isMid) { type = 'big'; hp = 800; w = 220; h = 150; imgSrc = BOSS_URLS.level5; }
    if (isFinal) { type = 'final'; hp = 1500; w = 300; h = 200; imgSrc = BOSS_URLS.level10; }
    ...
    this.boss = {
        ...
        lootTimer: 0, attackTimer: 0, currentAttack: 'idle'
    };
...
Game.spawnBossMinion = function(formation = 'V') {
    ...
    for(let i = 0; i < count; i++) {
        ...
        else if (formation === 'CROSS') { if (i < count/2) { offX = (i - count/4) * spacing; offY = 0; } else { offX = 0; offY = (i - 3*count/4) * spacing; } }
        ...
        this.enemies.push({
            ...
            hp: isHard ? 30 : 12, vy: isHard ? 4 : 2.5, vx: 0, 
            pathType: pType, originX: this.boss.x + offX,
            isMinion: true, imgSrc: this.boss.imgSrc,
            rot: 0, rotSpeed: 0.05
        });
    }
};
    ...
    if(boss.phase === 'fight') {
        ...
        if(boss.currentAttack === 'rapid') {
            if(boss.attackTimer % (isLevel10 ? 6 : 10) === 0) {
                ...
                if(isLevel5) this.spawnBossBullet(boss.x, boss.y + 50, angle + 0.2, 8); 
            }
            if(boss.attackTimer > 100) boss.currentAttack = 'idle';
        }

        if(boss.currentAttack === 'laser') {
            const charge = isLevel10 ? 30 : 60; 
...
Game.killBoss = function() {
    ...
    // 2. STOP ANANYA PARTNER (CRITICAL FIX FOR FREEZE)
    try {
        const partnerEl = document.getElementById('partner-entity');
        ...
        // Stop Divine Music Global Access
        if (Game.divineAudio) {
    ...
    this.slowMoTimeout = setTimeout(() => {
        ...
        if(this.foundLetters.every(x=>x)) { 
            this.winTimeout = setTimeout(() => this.nextLevel(), 1000); 
        } else { 
            AudioSys.speak("Finish the word!"); 
        }
    }, 2000);
};