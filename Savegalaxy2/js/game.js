...
// Load Sonic Boom Audio
const sonicBoomAudio = new Audio("https://nitnapple.github.io/Ananya/Savegalaxy/SonicBoom.mp3");
...
const Game = {
    ...
    // --- SONIC BOOM LOGIC (FIXED) ---
    triggerSonicBoom: function() {
        // Only active if Boss exists, Partner is alive, and Cooldown is ready
        if (!this.boss || (typeof this.partner !== 'undefined' && !this.partner.initialized) || this.specialCooldown > 0) return;

        // 1. Activate Cooldown (5 seconds)
        this.specialCooldown = 300; 
        this.sonicBoomActive = 40; 

        // 2. Visuals & Audio
        this.screenShake = 50;
        
        // --- USE CUSTOM MP3 ---
        sonicBoomAudio.currentTime = 0;
        sonicBoomAudio.play().catch(e => console.log(e));
        
        AudioSys.speak("Sonic Boom!");
        
        // 3. EFFECT: Destroy all Minions
    ...
    hitEnemy: function(e, index, damage) {
        ...
        if (e.hp <= 0) {
            ...
            if(e.isMinion) { 
                if (Math.random() < 0.20) { this.spawnPowerup(e.x, e.y); } 
                else if (Math.random() < 0.5) { this.spawnHeart(e.x, e.y); }
