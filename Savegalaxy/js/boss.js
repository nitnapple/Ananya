...
Game.spawnBoss = function() {
    ...
    this.boss = {
        ...
        // --- DROPS CONFIG ---
        dropThresholds: [0.75, 0.50, 0.25], // Drops at 75%, 50%, 25% HP
        lootTimer: 0, // Timer for 5-second recurring drops
...
Game.updateBoss = function(moveMult) {
    ...
    // --- 2. LOGIC: DROPS & DAMAGE ---
    const hpPct = boss.boss ? (boss.hp / boss.maxHp) : 0;
    
    // A. HP THRESHOLD DROPS (75%, 50%, 25%)
    if (boss.dropThresholds && boss.dropThresholds.length > 0) {
        if (hpPct < boss.dropThresholds[0]) {
            boss.dropThresholds.shift(); 
            // Spawn loot directly at boss location
            if (hpPct < 0.6 && hpPct > 0.4) {
                this.spawnHeart(boss.x, boss.y + 50);
            } else {
    ...
    // B. RECURRING 5-SECOND LOOT DROP (Powerup + Heart)
    if(boss.phase === 'fight') {
        boss.lootTimer++;
        // 300 frames approx 5 seconds at 60fps
        if(boss.lootTimer % 300 === 0) {
            // Spawn random powerup at top of screen (random X)
            this.spawnPowerup(Math.random() * (canvas.width - 50) + 25, -50);
            
            // Spawn Heart at top of screen (different random X)
            this.spawnHeart(Math.random() * (canvas.width - 50) + 25, -50);
            
            // Optional: Audio cue
            if(!AudioSys.isMuted) AudioSys.playCollect(true); 
        }
    }
