...
Game.updateBoss = function(moveMult) {
    ...
    // --- 2. DAMAGE SMOKE EFFECTS & DROPS ---
    const hpPct = boss.boss ? (boss.hp / boss.maxHp) : 0;
    
    // --- ADDED: DROP LOGIC ---
    if (boss.dropThresholds && boss.dropThresholds.length > 0) {
        // Check if we passed the next threshold
        if (hpPct < boss.dropThresholds[0]) {
            boss.dropThresholds.shift(); // Remove the threshold we just passed
            
            // At roughly 50% HP, spawn a Heart. Otherwise, spawn random powerup.
            // Since thresholds are 0.75, 0.50, 0.25, the middle one is 0.50.
            // We can check the HP range to determine the type.
